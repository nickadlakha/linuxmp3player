...
static void call_thread(int tfd)
{
    ...
    write(tfd,  start + 4, z - ((start + 4) - (char *)buf)); 
    ...
    while (1) {
        sres = select(sock + 1, &rset, NULL, NULL, NULL);
        
        if ((sres < 1) || (sres == 0)) break;
...
int main(int argc, char **argv)
{
    ...
    ((argv[1][0] == '-') && (strlen(argv[1]) == 1)) ?
        (res = mpg123_open_fd_64(mh, 0)) : (strstr(argv[1], "http://") ?
                                        return_sock(argv[1]) :
